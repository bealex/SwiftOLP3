...
The default chip is bit-exact integer. One experimental fork lives behind a compile flag (off by default): `OPL_BLOCKSIMD` (`OPL3BlockSimd.swift`), a **clean-room, DBOPL-architecture-inspired** block-generating SIMD float engine — state-dispatched envelope, idle-operator culling, and a branchless polynomial sine vectorised over the time axis. It is **~20 % faster on a real track / ~1.95× on sparse content**, accurate to ≈0.13 % RMS on tonal and ≈1 % on rhythm percussion (the noise-LFSR drums and the channel-sample-delay quirk are modelled). Because DOSBox's DBOPL is GPL and this package is LGPL, the engine had to be written from scratch — the full performance investigation **and the license analysis** are in `Performance.md`. (Two earlier forks, `OPL_FLOAT` and `OPL_SIMD`, were removed once this one superseded them; see git history.)
